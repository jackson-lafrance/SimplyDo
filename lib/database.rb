...
module Database
  def init_database
    ...
    @db.execute "CREATE TABLE IF NOT EXISTS Events(id INTEGER NOT NULL PRIMARY KEY, color INTEGER, title TEXT NOT NULL, description TEXT, start_date TEXT, length INTEGER,
times INTEGER, type TEXT)"
  ...
  def make_event(event)
    check_connection
    db.execute "INSERT INTO events (title, description, color, due_date) VALUES (?, ?, ?, ?)",
      event[:title], event[:description], event[:color], event[:due_date]
  end
